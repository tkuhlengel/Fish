...
def autothreshold(data, threshguess=None, high=None, low=None, highstd=4.0, 
    ...
    while(lastthresh != thresh and not math.isnan(thresh)):
...
def getObjectCenters(mask, labels=None, found_obj=None, structure=np.ones((3,3,3))):
    ...
    for i in found_obj:
        ...
        #print(com[0]+zslice.start, com[1]+rowslice.start, com[2]+colslice.start)
        #print(com)
        centers.append(com)
...
def binaryFillHoles(binarydata, kernel=np.ones((3,3,3))):
    ...
    if kernel is None:
        ...
        binary_fill_holes(binarydata, structure=kernel, output=result)
...
def binaryClosing(binarydata, structure=None, iterations=1):
    ...
    if structure is None:
        binary_closing(binarydata, iterations=iterations, output=result)
    else:
        binary_closing(binarydata, structure=structure, iterations=iterations, output=result)
    return result
...
def binaryOpening(binarydata, structure=None, iterations=3):
    ...
    if structure is None:
        binary_opening(binarydata, iterations=iterations, output=result)
    else:
        binary_opening(binarydata, structure, iterations=iterations, output=result)
    return result
